...
<%
    ...
    if(rs.next())out.println("<script>alert('该用户已存在，请重新注册！');window.location='2regist.jsp';</script>");
    else
    {
        PreparedStatement tmt = conn.prepareStatement("Insert into user values("+id+","+pw+","+name+","+sex+","+age+",null,"+tel+",null)");
        if(tmt.executeUpdate()!=0)out.println("<script>alert('用户注册成功！');window.location='..';</script>");
        else out.println("<script>alert('用户注册失败！');window.location='2regist.jsp';</script>");
    }
%>
</body>
