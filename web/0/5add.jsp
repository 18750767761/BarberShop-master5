...
<%
	...
	if(rs.next())out.println("<script>alert('该发型师已存在，请重新注册！');window.location='5barber.jsp';</script>");
	else
	{
		PreparedStatement tmt = conn.prepareStatement("Insert into barber values("+id+","+name+",null,null,"+s+","+pw+")");
		if(tmt.executeUpdate()!=0)out.println("<script>alert('发型师注册成功！');window.location='5barber.jsp';</script>");
		else out.println("<script>alert('发型师注册失败！');window.location='5barber.jsp';</script>");
	}
%>
